...
async def dev(kimo):
    await kimo.reply(
        "⌔ ⁝ 𝖦𝗂𝗍𝗁𝗎𝖻 𝖩𝖺𝗏𝖺",
        buttons=[[Button.url("🔗 𝖫𝗂𝗇𝗄", K)]]
    )
   

TOSH_PIC = Config.ALIVE_PIC if Config.ALIVE_PIC else "https://telegra.ph/file/8f59f7e12b0e1142f8458.jpg"
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("بوت") and event.query.user_id == bot.uid:
            buttons = [
                [
                    Button.url("الرابط 🔗","https://github.com/JAI6H"),
                    Button.url("المطور ⚙️", "https://t.me/GI53H"),
                ]
            ]
